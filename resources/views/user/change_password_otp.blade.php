...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
   <head>
      ...
      <title>Wealth Mark | Change Password OTP</title>
               ...
               <div class="container step-2">
                    <div class="row">
                        <div class="col-lg-4 col-md-6 offset-md-1 offset-lg-2">
                            <h3 class="form-heading mb-4 text-center">
                               Security verification
                            </h3>
                            <div class="row">
                               <div class="col-md-12">
                                    <div class="reset-bg p-3 d-flex align-items-center rounded mb-2">
                                    ...
                                    <form action="" method="post" id="email_otp_frm">
                                        ...
                                        <div class="verification_div">
                                            ...
                                            <div class="verification_innerInfo  error_text">Enter the 6-digit code sent to <span  dir="ltr" class="getCode_outer1">{{ substr(auth()->user()->phone, 0, 3) . "***" . substr(auth()->user()->phone, 6, 4) }}</span> </div>
                                        </div>
                                        ...
                                        <div class="verification_div mt-3">
                                            ...
                                            <div class="verification_innerInfo  error_text">Enter the 6-digit code sent to <span  dir="ltr" class="getCode_outer1">{{ hideEmailAddress(auth()->user()->email) }}</span>
                                            </div>
                                        </div>
                                        <div class="my-3"><span class="text-muted"><a href="" class="text-theme-yellow"> Security verification unavailable?</a></span></div>
                                        <div class="btn-div">
                                            <a href="#" class="submit_code w-100 mt-3" id="submit_otp">
            	...
            	$(document).ready(function () {
            		$("#submit_otp").click(function() {
            			...
            			if(p_count==6 && e_count==6){
            				$.ajax({
            					type: "POST",
                                url: '{{ url(app()->getLocale())}}/user/submit_change_pass_otp',
                                data: {'email_otp':e_otp,'phone_otp':p_otp,'_token':token},
            					success: function(data) {
            					    if(data.success==200){
            					        $('#email_otp_frm').trigger("reset");
                                        triggerAlert('Congratulations! your password successfully changed.', 'success');
                                        setTimeout(function(){
                                            window.location.href = '{{url( app()->getLocale()) }}/user/security';
                                        }, 3000);
            					    }else{
            					        triggerAlert('Sorry! something went wrong.', 'error');
            					    }
                                },
                                   ...
                                   var error = erroJson.error;
                                    if(error.phone_otp){
                                     $('.potp_verification_inner ').addClass('error_border');
                                      $("#p_otp_err").html(error.phone_otp);
                                    }
                                    if(error.email_otp){
                                      $('.eotp_verification_inner ').addClass('error_border');
                                      $("#e_otp_err").html(error.email_otp);
                                    }
                                }
            			    ...
            			    if(p_count==""){
            			        ...
            			        if(p_count!=6){
                			        $('.potp_verification_inner').addClass('error_border');
                                    $("#p_otp_err").html("Please enter valid phone otp");
                			    }
            			    }
            			    ...
            			    if(e_count==""){
            			        ...
            			        if(e_count!=6){
                			        $('.eotp_verification_inner ').addClass('error_border');
                                    $("#e_otp_err").html("Please enter valid email otp");
                			    }
            			    }
            			}
            		});
