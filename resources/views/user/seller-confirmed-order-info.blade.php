...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-white seller_from_sell_tab">
                   ...
                   <div class="col-9">
                        <div class="d-md-flex justify-content-between align-items-center if-order-cancel">
                            <div class="col-8">
                                ...
                                <span>95% of the Seller's order have been completed within</span>
                            </div>
                            <!--<div id="timer"></div> -->
                            <div class="col-4">
                                <div class="order_no">
                                ...
                                <div class="time_created">
                                   ...
                                   <span> {{ App\Http\Controllers\HomeController::fetchDateFromSQL($get_order_data->created_at) }}</span>
                                </div>
                            </div>
                        </div>
                   </div>
        ...
        <section>
                             ...
                             @if(!empty($buyer_details) && $get_order_data->order_status==1)
                               <div class="ordered-user-info">
                                    <div class="bg-white p-3">
                                        <h5 class="fs-14">
                                            <span><?php echo $buyer_details->first_name; ?>
            ...
            <!--<div class='xyz' id="xyz1122" type="button">click me</div>-->
        </section>
        ...
        <!-------------------- Commented Script -------------------->
        <!-- <script>
            // this script related to appeal functionality
            ...
            var check_seller_order = setInterval(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if(data.sellerDetail.order_status == 1 && data.sellerDetail.order_accept_action == 1){
                ...
                $(document).ready(function(){
                    ...
                    $("#request_crypto_transfer").click(function(){
                        ...
                        $.ajax({
                            ...
                            data: {
                                "_token": "{{ csrf_token() }}",
                                request_crypto_transfer:request_crypto_transfer
        ...
        @if($get_order_data->order_status == 1)
            @if($get_order_data->order_accept_action == 1)
...
//     }
    ...
    //     if (orderStatus == 1 && buyer_confirmation_status == null) {
    //         var timelocal = setInterval(function() {
    //         var timerState = {
    //             counter: counter
    //         };
    ...
    //         var counter = timerState.counter;
    //         offtimer = setInterval(function() {
    //             counter--;
    //             var minutes = Math.floor(counter / 60);
    //             var seconds = counter % 60;
    //             if (counter >= 0) {
    //                 //$('#timer').text('95% of the buyer\'s order have been completed within ' + minutes +' minutes' + ' : ' + (seconds < 10 ? '0' : '') + seconds + ' seconds');
    //                 $('#timer').html('95% of the buyer\'s order have been completed within <span style="font-weight:bold; background-color:rgb(250 173 40);">' + minutes +' minutes</span>' + ' : ' + '<span style="font-weight:bold; background-color: rgb(250 173 40);">' + (seconds < 10 ? '0' : '') + seconds + ' seconds</span>');

    //             }
    //             if (counter === 0) {
    //                 clearInterval(offtimer);
    //                 clearInterval(timelocal);
    ...
    //      }
    //     // }
    //     // else if(orderStatus == 1 && buyer_confirmation_status == 1) {
    ...
    //     //         var seconds = counter % 60;
    //     //         if (counter >= 0) {
    //     //             $('#timer').text('Order will be cancelled in ' + minutes +' minutes' + ' : ' + (seconds < 10 ? '0' : '') + seconds + ' seconds');
    //     //         }
    //     //         if (counter === 0) {
    //     //             clearInterval(offtimer);
    ...
    //         if (response.status.order_status == 1 && response.status.buyer_confirmation_status != null) {
    //             $('#timer').text('');
    //             clearInterval(offtimer);
    //             clearInterval(timelocal);
    //         }
    //         if (response.status.order_status == 1 && response.status.buyer_confirmation_status == 1) {
    //             $('#timer').text('');
    //             clearInterval(offtimer);
    //             clearInterval(timelocal);
    //         }
    //          if (response.status.order_status == 4 && response.status.buyer_confirmation_status == null) {
    //             $('#timer').text('');
    //             clearInterval(offtimer);
    //             clearInterval(timelocal);
    //         }
    ...
    //   },1000);
        ...
        <script>
            ...
            function update(percent) {
                ...
                if (percent < (remainingTime / 2)) {
                    ...
                    deg = -90 + (360 * percent / remainingTime);
                    $('.pie').css('background-image', 'linear-gradient(' + deg + 'deg, transparent 50%, #fec00f 50%),linear-gradient(90deg, white 50%, transparent 50%)');
        ...
        //     var buyer_confirmation_status = <?php // $buyer_confirmation_status = $get_order_data->buyer_confirmation_status;
        //                                         if(!empty($buyer_confirmation_status)) {
        ...
        //     var order_accept_action =   <?php // $order_accept_action = $get_order_data->order_accept_action;
        ...
        //     var seller_confirmation_status =    <?php // $seller_confirmation_status = $get_order_data->seller_confirmation_status;
        ...
        // }
            ...
            return new Promise(function(resolve, reject) {
                ...
                $.get('{{ route('check-order-status', ['id' => ':id']) }}'.replace(':id', orderId), function(response) {
                    ...
                    const order_accept_action = response.status.order_accept_action

                    if((response.status.order_status == 1 && response.status.order_accept_action == 1)){
                        // startTime()
                    } else if (response.status.order_status == 1 && response.status.buyer_confirmation_status == 1 && response.status.order_accept_action== 1 && response.status.seller_confirmation_status== 1) {
            ...
            // to check when buyer canceled the order and selected if he did it due to seller note is created but class is not created on controller at 27042024
            // $.ajax({
                                //         url: '{{ route('IfBuyerCanceledOrderDueToSeller') }}',
                                //         method: 'POST',
                                //         data: {
                                //         '_token' : '{{ csrf_token() }}',
                                //         'orderid' : '<?php echo $_SESSION['orderid']; ?>',
                                ...
                                //      });
            // end to check if buyer canceled the order and selected if he did it due to seller
        }
            ...
            $(document).ready(function(){
                ...
                function startTimer(orderId, payment_time_limit) {
                    $.ajax({
                        ...
                        success: function(data) {
                            if(data.orderObj.order_status == 1 && data.orderObj.order_accept_action == 1){
                                if(response.status.seller_confirmation_status== 1 || response.status.buyer_confirmation_status == 1) {
                                    var minutes = Math.floor(payment_time_limit / 60);
                                    var seconds = payment_time_limit % 60;
                                    $('#timer').html('');
                                    $('#timer').html(minutes + ':' + (seconds < 10 ? '0' : '') + seconds);

                                } else if(response.status.seller_confirmation_status== 1 && response.status.buyer_confirmation_status == 1){
                                    triggerAlert('Order Completed!', 'succes');
                                   clearInterval(timeInterval);
                                } else {
                                    var minutes = Math.floor(payment_time_limit / 60);
                                    var seconds = payment_time_limit % 60;
...
//   var elapsed = Date.now()- startTimer;
//   console.log(elapsed);
//   var minutes = Math.floor(elapsed/60000);
//   var seconds = Math.floor((elapsed%60000)/1000);
//   console.log("===============",minutes +":"+(seconds < 10 ? '0' : '') +seconds);
//   timer = setTimeout(updateTimer, <?php // echo $get_order_data->payment_time_limit; ?>);
// }
//  function stopTimer(){
//      clearTimeout(timer);
//  }
