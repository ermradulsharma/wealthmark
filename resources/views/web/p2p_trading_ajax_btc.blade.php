<div class="tab-pane {{ (Request::segment(3) == 'btc') ? 'show active':'fade'}}" id="p2p-trading-BTC" role="tabpanel" aria-labelledby="p2p-trading-BTC-tab">
    ...
    <div class="tokens-table ">
                ...
                @foreach($get_BTC_orders as $btc)
                 @php
                    $username = App\Models\User::where('id',$btc->buyer_id)->first();
                    $countbtc_orders = App\Models\Order::where('buyer_id',$btc->buyer_id)->where('crypto_type',"$btc->crypto_type")->get();
                    $count = $countbtc_orders->count();
                    $confirmorder = App\Models\Order::where('buyer_id',$btc->buyer_id)->where('crypto_type',"$btc->crypto_type")->where('order_status',1)->get();
                    $count_confirmorder = $confirmorder->count();
                    $cancel_order = App\Models\Order::where('buyer_id',$btc->buyer_id)->where('crypto_type',"$btc->crypto_type")->where('order_status',3)->get();
                    ...
                    $payment_method_type = App\Models\payment_method_list::where('id', $btc->payment_method_id)->first();
                     // $currencies=  App\Models\currencies::where('id', $btc->domestic_currency_type)->first();
                     ...
                     $base_url=env('BMK_API_BASE_URL');
                            ...
                            $currencies = $allcurrency->where('id', $btc->domestic_currency_type)->first();
                            $currenciesname = $currencies->name;
                 @endphp
                  <div class="p2p-tbl-record-body" id="{{$btc->id}}">
                            <div class="block one">
                            ...
                            <div class="block two">
                                ...
                                <div>
                                    <span class="buyer_cr_value">{{$btc->domestic_currency_value}} </span>
                            ...
                            <div class="block five">
                                ...
                                <div class="buyer_Crypto_type">
                                 <a href="javascript:void(0);" onclick="sell_btc({{$btc->id}})" class="btn-green">Sell {{$btc->crypto_type}}</a>
                           </div>
                            </div>
                        </div>
    ...
    <div class="d-flex justify-content-center w-100 pg-nation">
        {!! $get_BTC_orders->render('pagination::bootstrap-5') !!}
    </div>
</div>
</div>
